...
python -m zero.expLongHorizon.trainer_expbase \
        --exp-config ./zero/expLongHorizon/config/expBase_Lotus.yaml \
        name $name dataset augment num_gpus 1 \
        epoches 200 \
        batch_size 4 \
        ...
        TRAIN.n_workers 4 MODEL.action_config.action_head_type multihead \
        B_Preprocess ./1_Data/B_Preprocess/0.005all_with_path \
        horizon 8 \
...
exp_dir=./zero/2_Train/$name/version_0
python -m zero.expLongHorizon.eval_LongHorizon \
        --exp-config ./zero/expLongHorizon/config/eval.yaml exp_dir $exp_dir \
        headless True \
        num_workers 4 \
        tasks_to_use $tasks_to_use
