...
python -m zero.expForwardKinematics.trainer_expbase \
        --exp-config ./zero/expForwardKinematics/config/expBase_Lotus.yaml \
        name EXP03_04_insert_close_jar_0.005 dataset augment num_gpus 1 \
        epoches 1800 \
        batch_size 1 \
        ...
        MODEL.action_config.voxel_size 0.005 MODEL.action_config.euler_resolution 1 tasks_to_use "$tasks_to_use" \
        TRAIN.n_workers 4 B_Preprocess ./1_Data/B_Preprocess/0.005all \
