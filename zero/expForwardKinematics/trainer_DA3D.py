...
from zero.expForwardKinematics.dataset.dataset_DA3D import DatasetDA3D as Dataset
from zero.expForwardKinematics.dataset.dataset_DA3D import collect_fn
from zero.expForwardKinematics.models.DiffuserActor3D.Policy import Policy
...
class TrainerDA3D(pl.LightningModule):
...
def train(config: yacs.config.CfgNode):
    ...
    config.freeze()
    trainer_model = TrainerDA3D(config)
