...
def main():
    ...
    args.exp_config = '/data/zero/zero/v1/config/lotus.yaml'
    ...
    for i in range(20):
        ...
        result_queues = [mp.Queue(args.queue_size) for _ in range(args.num_process)]
        ...
        while i < len(taskvars):
            taskvar = taskvars[i]
            if len(producers) < args.num_process:  # 如果有空余的process,创建producer
