...
def get_disc_gt_pos_prob(
    ...
    if bin_strategy == 'nonlinear':
        ...
        # given bin_size, BIN_SPACE, bin的数量就是固定的，因为binspace的长短是固定的，
        ...
        # 3，normalize
        x_prob[x_dist > pos_bin_size] = 0  # TODO：这里的距离阈值是多少？先设置成这个吧
        y_prob[y_dist > pos_bin_size] = 0
        z_prob[z_dist > pos_bin_size] = 0

        x_prob = x_prob / np.sum(x_prob)
        y_prob = y_prob / np.sum(y_prob)
        z_prob = z_prob / np.sum(z_prob)

        disc_pos_prob = np.hstack([x_prob, y_prob, z_prob])  # (3, pos_bins)

        # 可以stack，反正后面可以解码，只要知道bin_size和bin_space就行
