...
class TestPolicy(BasePolicy):
    def __init__(self, config):
        super().__init__()
        self.ActionHead = ActionHead(config).to('cuda')
        self.FeatureExtractor = FeatureExtractor(config).to('cuda')
...
def test():
    ...
    example_data = '/media/jian/ssd4t/zero/1_Data/C_Dataset_Example/example.pkl'
