...
from zero.expAugmentation.models.DiffuserActor3D.ActionHead import ActionHead, ActionHead_Original
from zero.expAugmentation.models.DiffuserActor3D.FeatureExtractor import FeatureExtractor, FeatureExtractor_Original
...
class Policy(BasePolicy):
    def __init__(self, config):
        ...
        self.use_instruction = config['DiffuserActor']['use_instruction']

        self.timestep_scheduler = DDPMScheduler(
            num_train_timesteps=config['DiffuserActor']['Policy']['num_train_timesteps'],
...
class FeatureExtractor(nn.Module):
    ...
    def forward(self, rgb, pcd, instruction, action):
        ...
        joint_position = normalize_theta_positions(joint_position)
        ...
        # encode action
        his_action = self._encode_gripper(action, self.action_hist_embed, context_feats, context)
