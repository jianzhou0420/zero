...
from zero.expAugmentation.models.diffuser_actor.utils.layers import (
    FFWRelativeSelfAttentionModule,
    FFWRelativeCrossAttentionModule,
    FFWRelativeSelfCrossAttentionModule
)
from zero.expAugmentation.models.diffuser_actor.utils.encoder import Encoder
from zero.expAugmentation.models.diffuser_actor.utils.layers import ParallelAttention
from zero.expAugmentation.models.diffuser_actor.utils.position_encodings import (
    RotaryPositionEncoding3D,
    SinusoidalPosEmb
)
from zero.expAugmentation.models.diffuser_actor.utils.utils import (
...
class DiffuserActor(nn.Module):
    ...
    def encode_inputs(self, visible_rgb, visible_pcd, instruction,
                      curr_gripper):
        visible_pcd = visible_pcd.to('cuda')
        visible_rgb = visible_rgb.to('cuda')
        instruction = instruction.to('cuda')
        curr_gripper = curr_gripper.to('cuda')
        # Compute visual features/positional embeddings at different scales
        rgb_feats_pyramid, pcd_pyramid = self.encoder.encode_images(
            visible_rgb, visible_pcd
        )
        # Keep only low-res scale
        vision_language_features = einops.rearrange(
            rgb_feats_pyramid[0],
            "b ncam c h w -> b (ncam h w) c"
        )
        assigned_postion = pcd_pyramid[0]  # 每个feature的xyz

        # Encode instruction (B, 53, F)
        instruction_features = None
        if self.use_instruction:
            instruction_features, _ = self.encoder.encode_instruction(instruction)

        # Cross-attention vision to language
        if self.use_instruction:
            # Attention from vision to language
            vision_language_features = self.encoder.vision_language_attention(
                vision_language_features, instruction_features
            )

        # Encode gripper history (B, nhist, F)
        adaln_gripper_feats, _ = self.encoder.encode_curr_gripper(
            curr_gripper, vision_language_features, assigned_postion
        )

        # FPS on visual features (N, B, F) and (B, N, F, 2)
        fps_feats, fps_pos = self.encoder.run_fps(  # 最远点采样，干什么要这么采样？
            vision_language_features.transpose(0, 1),
            self.encoder.relative_pe_layer(assigned_postion)
        )
        return (
            vision_language_features, assigned_postion,  # contextualized visual features
            instruction_features,  # language features
    ...
    def forward(
        ...
        Arguments:
            ...
            curr_gripper: (B, nhist, 3+4+X) # what does x mean?
        ...
        if gt_trajectory is not None:
            gt_trajectory = gt_trajectory.to('cuda')
...
class DiffusionHead(nn.Module):
    ...
    def forward(self, trajectory, timestep,
        ...
        """
        # Trajectory features
        trajectory = trajectory.to(context_feats.device)
