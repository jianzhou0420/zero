...
class Encoder(nn.Module):
    ...
    def _encode_gripper(self, gripper, gripper_embed, context_feats, context):
        ...
        Args:
            - gripper: (B, npt, 3+) [1,3,9] [b,n_history, 9] 9是什么未知从下文可以看出前3是xyz
    ...
    def encode_images(self, rgb, pcd):
        ...
        Args:
            ...
            - pcd: (B, ncam, 3, H, W), positions (point cloud)
    ...
    def encode_instruction(self, instruction):
        ...
        Args:
            - instruction: (B, max_instruction_length, 512) # tokenized instruction

        Returns:
            - instr_feats: (B, 53, F), max_instruction_length=53
            - instr_dummy_pos: (B, 53, F, 2)
        """
        instr_feats = self.instruction_encoder(instruction)  # jian: Embedding layer
        # Dummy positional embeddings, all 0s
        instr_dummy_pos = torch.zeros(  # bs, max_instruction_length, 3
            len(instruction), instr_feats.shape[1], 3,
            device=instruction.device
        )

        instr_dummy_pos = self.relative_pe_layer(instr_dummy_pos)  # jian: PE layer
