...
class ObsProcessLotus:
    ...
    def __init__(self, rm_pc_outliers_neighbors=25, config=None, selfgen=True):
    ...
    def augment_pc(self, flag, xyz, ee_pose, gt_action, gt_rot, aug_max_rot, rot_type, euler_resolution):
        if flag:
            ...
            ee_pose[3:-1] = self.rotate_gripper(ee_pose[3:-1], angle)
            gt_action[3:-1] = self.rotate_gripper(gt_action[3:-1], angle)
            if rot_type == 'quat':
