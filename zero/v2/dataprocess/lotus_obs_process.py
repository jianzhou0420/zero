...
class ObsProcessLotus:
    ...
    def augment_pc(self, xyz, action_current, action_next, gt_rot, aug_max_rot, rot_type, euler_resolution):
        '''
    ...
    def quaternion_to_type(self, rot, rot_type):
        if rot_type == 'quat':
            ...
            gt_rot = quaternion_to_discrete_euler(rot, self.config.TRAIN_DATASET.euler_resolution)
        elif rot_type == 'rot6d':
            gt_rot = self.rotation_transform.quaternion_to_ortho6d(
    ...
    def pc_action_standard_process(self, xyz, rgb, arm_links_info, action_current, action_next=None, is_train=False):
        ...
        xyz, rgb = self.downsample(xyz, rgb, action_current, sample_points_by_distance=self.config.TRAIN_DATASET.sample_points_by_distance, num_points=self.config.TRAIN_DATASET.num_points, downsample_type='random')
        ...
        #     angle = np.random.uniform(-1, 1) * self.config.TRAIN_DATASET.aug_max_rot
        ...
        # 1.convert action_next's quaternion to discrete euler
            action_next_rot = quaternion_to_discrete_euler(action_next[3:-1], self.config.TRAIN_DATASET.euler_resolution)
        ...
        # 2.get gt_pos_prob 是最后再做的，约等于转换以下action_next
            disc_pos_prob = self.action_next_pos_prob(
                xyz, action_next[:3], pos_bins=self.config.TRAIN_DATASET.pos_bins,
    ...
    def dataset_generation_single_episodes(self, data, voxel_size, episode_path, instr_embeds, taskvar_instrs, visualize=False):
      ...
      outs = {
        ...
        for t in range(len(data['key_frameids']) - 1):  # last frame dont train
            ...
            outs['action_currents'].append(torch.from_numpy(action_current).float())
    ...
    def _get_mask_with_robot_box(self, xyz, arm_links_info, rm_robot_type):
        ...
        robot_box = RobotBox(
            ...
            env_name='rlbench', selfgen=self.selfgen
        )
        _, robot_point_ids = robot_box.get_pc_overlap_ratio(xyz=xyz, return_indices=True)
        robot_point_ids = np.array(list(robot_point_ids))
        mask = np.ones((xyz.shape[0], ), dtype=bool)
    ...
    def new_dataset_generation_single_episodes(self, data, voxel_size, episode_path, instr_embeds, taskvar_instrs, visualize=False):
        ''' #TODO: voxel_size is not used
        receive data from selfgen_one_step, this function process single episode data
        input = {
            'key_frameids': [],
      ...
      outs = {
        ...
        for t in range(len(data['key_frameids']) - 1):  # last frame dont train
            ...
            instr_embed = instr_embeds[instr]
            gt_rot = gt_rots[t]
            # 3. voxelization
            pcd = o3d.geometry.PointCloud()
            pcd.points = o3d.utility.Vector3dVector(xyz)
            ...
            # 4. remove none object points: unused here, but eval_origin.py 中有用到
            # if not self.args.real_robot:
            #     rm_label_ids = get_rlbench_labels(
            #     taskvar.split('+')[0], table=self.config.TRAIN_DATASET.rm_table, robot=(self.config.TRAIN_DATASET.rm_robot == 'gt'), wall=False, floor=False
            # )
            ...
            aug_max_rot = self.config.TRAIN_DATASET.aug_max_rot
            aug_max_rot = np.deg2rad(aug_max_rot)
            xyz, action_current, action_next, gt_rot = self.augment_pc(
                xyz, action_current, action_next, gt_rot,
                aug_max_rot,
                self.config.TRAIN_DATASET.rot_type,
                self.config.TRAIN_DATASET.euler_resolution)

            # 7. normalize xyz, action_current, action_next, gt_rot, aug_max_rot, rot_type, euler_resolution
            ...
            if self.config.TRAIN_DATASET.get('use_height', False):
    ...
    def new_dataset_generation(self, origin_data_root, output_dir, tasks_to_use=None):
        voxel_size = 0.005
        tasks_list = self.retrieve_all_episodes(origin_data_root)
        taskvar_instr_file = self.config.TRAIN_DATASET.taskvar_instr_file
        instr_embed_file = self.config.TRAIN_DATASET.instr_embed_file
            ...
            for variation in task:
                for episode in variation:
                    ...
                    with open(data_path, 'rb') as f:
                        data = pickle.load(f)
                    # print(rgb.shape, xyz.shape)
                    new_data = self.new_dataset_generation_single_episodes(data, voxel_size, episode, instr_embeds, taskvar_instrs, visualize=False)
