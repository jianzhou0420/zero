...
if __name__ == '__main__':
    # Step 1: Create a new ReplayBuffer (in-memory or on-disk)
    # Option 1: In-memory Zarr-based buffer
    # Step 1: 创建一个 ReplayBuffer (使用磁盘存储)
    # 假设数据存储在 zarr 文件中，数据量很大
    zarr_path = './my_replay_buffer.zarr'

    # 创建一个 Zarr 存储对象，将数据存储在磁盘上
    replay_buffer = ReplayBuffer.create_from_path(zarr_path, mode='a')

    # Step 2: 假设每个 episode 包含多个 time steps，每个 time step 包含多个数据项
    # 例如，每个 episode 包含 10000 步，每步有 10 个数据点
    episode_length = 10000  # 每个 episode 包含 10000 步
    num_features = 10  # 每步包含 10 个特征
    num_episodes = 100  # 总共有 100 个 episode

    # Step 3: 生成并添加数据到 ReplayBuffer
    # 每个 episode 的数据可以是随机生成的，实际应用中应使用真实数据
    for episode_idx in range(num_episodes):
        episode_data = {
            'observations': np.random.random((episode_length, num_features)),  # 随机生成观察数据
            'actions': np.random.randint(0, 5, size=(episode_length,)),  # 随机生成行动
            'rewards': np.random.random((episode_length,))  # 随机生成奖励
        }

        # 将该 episode 的数据添加到 ReplayBuffer
        replay_buffer.add_episode(episode_data)

    # Step 4: 获取某个 episode 的数据
    episode_idx = 0  # 获取第 0 个 episode
    retrieved_episode = replay_buffer.get_episode(episode_idx)

    # 打印获取的 episode 数据
    print("Episode Observations:\n", retrieved_episode['observations'])
    print("Episode Actions:\n", retrieved_episode['actions'])
    print("Episode Rewards:\n", retrieved_episode['rewards'])

    # Step 5: 获取所有 episode 的总步数和总 episode 数
    print(f"Total steps in the buffer: {replay_buffer.n_steps}")
    print(f"Total episodes in the buffer: {replay_buffer.n_episodes}")

    # Step 6: 存储到磁盘
    replay_buffer.save_to_path(zarr_path)  # 保存数据到指定路径

    # Step 7: 重新加载 ReplayBuffer（从磁盘）
